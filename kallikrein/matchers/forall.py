...
from kallikrein.match_result import MatchResult, SimpleMatchResult, ForAllMatchResult
...
class ForAll(Generic[A], Matcher[Collection[A]]):
    ...
    def match(self, exp: Collection[A], target: A) -> MatchResult[Collection[A]]:
    ...
    def match_nested(self, exp: Collection[A], target: Matcher) -> MatchResult[Collection[A]]:
